...
!    logical, parameter :: do_methane_fcast = .True.
        logical, parameter :: do_methane_fcast = .False.

      logical,parameter :: do_EBG = .True.
!      logical,parameter :: do_EBG = .False.
...
!    climatefile1='input/SPRUCE_forcing2022_PBOT.txt'
!    patmfile='input/2011-2016forcing_PBOT.txt'  ! used in methane module only
! *************** climate file for do_methane_fcast **************************
!    climatefile1='input/Weathergenerate_PBOT/2011-2024forcing1516repeat/forcing2011-2024_0.txt'
! ****************************************************************************
...
!   ..int read obs files for CO2, soil thermal and soil water DA
    call GetObsData(obs_spruce,std,obs_soilwater,obs_CH4_MEMCMC,std_CH4,     &
            &   len1,len2,obsfile1,obsfile2,obs_soilt,obsfile3,obs_watertable_d,obs_soilt_d,&
            &   obsfile4,obsfile5,obsfile6,len3,len4,len5)
...
!    QC=(/450.,380.,250.,119.,300.,322.,38340.,23120./)      ! updated leave wood biomass due to the change of estimated plot area
    QC=(/300.,380.,250.,119.,300.,322.,38340.,23120./)  ! updated moss leave C mass, in accordance with 2016-2017 data * 0.6 for active leaves
        QC=(/300.,500.,250.,200.,300.,322.,28340.,23120./)
!        QC=(/300.,380.,250.,119.,300.,200.,38340.,23120./)
!   Start main loop
!    call getarg(4,outdir)
    outdir = 'output'

!        open(86,file='TECO_validate_methaneinput.csv')
!        write(86,*)'zwt,Tsoil10,Rh_pools(1),Rh_pools(2),Rh_pools(3),Rh_pools(4),Rh_pools(5),&
!          & wsc(1),wsc(2),wsc(3),wsc(4),wsc(5),wsc(6),wsc(7),wsc(8),wsc(9),wsc(10), &
!          & Tsoil1,Tsoil2,Tsoil3,Tsoil4,Tsoil5,Tsoil6,Tsoil7,Tsoil8,Tsoil9,Tsoil0,dpatm,LAIMAX,LAIMIN,phi'
    ...
    if (.not. do_co2_da .and. .not. do_methane_da) then
        if (.not. do_methane_fcast) then
            ...
            write(62,*)'sdoy, GPP_d, NEE_d, Reco_d, QC6, GL_yr, QC7, GW_yr, QC8, GR_yr, QC678, pheno, LAI'
    ...
    !        write(662,*)'sdoy, GPP_d, NEE_d, Reco_d, NPP_d, Ra_d, QC1, QC2, QC3, QC4, QC5, QC6, QC7, QC8, Rh_d'
            write(662,*)'sdoy, GPP_d, NEE_d, Reco_d, NPP_d, Ra_d, QC1, QC2, QC3, add, storage, LAIMAX, Rh_d, LAI, QC4'
        ! end of inserting
...
!            write(outfile,"(A120,A28)") trim(outdir),"/Simu_hourly_Methane_EBG.csv"
!            outfile = trim(outfile)
!            outfile = adjustl(outfile)
!            open(69,file=outfile)
!            write (69,*) 'CH4(1),CH4(2),CH4(3),CH4(4),CH4(5),CH4(6),CH4(7),CH4(8),CH4(9),CH4(10), &
!                & CH4_V(1),CH4_V(2),CH4_V(3),CH4_V(4),CH4_V(5),CH4_V(6),CH4_V(7),CH4_V(8),CH4_V(9),CH4_V(10),   &
!                & Vp(1),Vp(2),Vp(3),Vp(4),Vp(5),Vp(6),Vp(7),Vp(8),Vp(9),Vp(10),  &
!                & mebu_rateP(1),mebu_rateP(2),mebu_rateP(3),mebu_rateP(4),mebu_rateP(5),mebu_rateP(6),mebu_rateP(7), &
!                & mebu_rateP(8),mebu_rateP(9),mebu_rateP(10),  &
...
!     added for both DA and forecasting: npara, and coefindex. added when developing forecasting for methane 09/14/2018
    if (do_methane_fcast) then  ! if do fcast and draw parameters from uniform distribution
        npara=sum(DApar)
...
!    if(MCMC.eq.1) GOTO 100
    if (do_co2_da .or. do_methane_da) GOTO 110          ! if do any DA goto 100
!    if (MCMC.eq.2) GOTO 150
    if (do_methane_fcast)  GOTO 150
    year_seq = year_seq1
    doy_seq = doy_seq1
    hour_seq = hour_seq1
    ...
    yrs_eq=yr_length*0  ! spin up length edit there on Nov2018
    call TECO_simu(MCMC,Simu_dailyflux,Simu_soilwater,obs_soilwater,      &
     ...
     &        do_snow,do_soilphy,do_co2_da,do_methane_da,do_methane_fcast,do_EBG,output_data,dpatm)
...
!    write(paraestfile,"(A120,A21)") trim(outdir),"/Paraest_flux_old.txt"
!        write(paraestfile,"(A120,A25)") trim(outdir),"/Paraest_flux_EBG_new.txt"
                write(paraestfile,"(A120,A29)") trim(outdir),"/Paraest_fluxconc_EBG_new.txt"
!    write(paraestfile,"(A120,A21)") trim(outdir),"/Paraest_fluxconc_old.txt"
!        write(paraestfile,"(A120,A21)") trim(outdir),"/Paraest_fluxconc.txt"
...
!!   #### generate a uniform distribution for no data informed forecast
!        CALL random_number(randnum)
!        write (*,*) 'random',randnum
!        r_me = parmin(36)+randnum*(parmax(36)-parmin(36))
!        CALL random_number(randnum)
!        write (*,*) 'random',randnum
!        Q10pro = parmin(37)+randnum*(parmax(37)-parmin(37))
!        CALL random_number(randnum)
!        write (*,*) 'random',randnum
!        Omax = parmin(39)+randnum*(parmax(39)-parmin(39))
!        CALL random_number(randnum)
!        write (*,*) 'random',randnum
!        Tveg = parmin(41)+randnum*(parmax(41)-parmin(41))
!        write (*,*) 'random',randnum
!        f = parmin(44)+randnum*(parmax(44)-parmin(44))
!        write (*,*) 'random',randnum
!        bubprob = parmin(45)+randnum*(parmax(45)-parmin(45))
!        write (*,*) 'random',randnum
!        Vmaxfraction = parmin(46)+randnum*(parmax(46)-parmin(46))
!!   end of generating a uniform distribution
...
!!!################  changed mind and run as DA  #################################################!
!
!!   Read generated climatic forcing
!!    call getarg(7,forcingdir)
!    forcingdir = 'input/Weathergenerate_PBOT/2011-2024forcing1516repeat'
!!    write(climatefile2,"(A120,A15,I3.3,A4)") trim(forcingdir),"/EMforcing_PBOT",rep,".csv"
!!    write(climatefile2,"(A120,A15,A3,A4)") trim(forcingdir),"/EMforcing_PBOT",'amb',".csv"
!    climatefile2='input/Weathergenerate_PBOT/2011-2024forcing1516repeat/forcing2011-2024_0.txt'
...
!!    do k1=1,lines1
!!        year_seq(k1)=year_seq2(k1)
!!        doy_seq(k1)=doy_seq2(k1)
!!        hour_seq(k1)=hour_seq2(k1)
!!        forcing_data(1,k1)=forcing_data2(1,k1)
!!        forcing_data(2,k1)=forcing_data2(2,k1)
!!        forcing_data(3,k1)=forcing_data2(3,k1)
!!        forcing_data(4,k1)=forcing_data2(4,k1)
!!        forcing_data(5,k1)=forcing_data2(5,k1)
!!        forcing_data(6,k1)=forcing_data2(6,k1)
!!        forcing_data(7,k1)=forcing_data2(7,k1)
!!    enddo      !this do loop is not necessary
!        year_seq=year_seq2
!        doy_seq=doy_seq2
!        hour_seq=hour_seq2
!        forcing_data=forcing_data2
!        lines=lines2
!        yr_length=yr_length2
!!    write (*,*) 'year_seq',year_seq(34),'doy_seq',doy_seq(34),'hour_seq',hour_seq(34),'forcing_data',forcing_data(:,34),&
!!    &   'lines',lines
!!!################  changed mind and run as DA  #################################################!
    year_seq = year_seq1
    doy_seq = doy_seq1
...
!   ****************** recover initial value after each forecasting run ***************************
    fwsoil=1.0
    topfws=1.0
    omega=1.0
    do i=1,10
    ...
    Storage=60.0 !32.09           !g C/m2
    nsc=85.35
    QC=(/300.,380.,250.,119.,300.,322.,38340.,23120./)
!   *****************************************************
...
! ..int CWE version cost function
        call costFObsNee(Simu_dailyflux,Simu_soilwater,   &
            ...
            &   Simu_dailysoilt,Simu_dailywatertable,obs_soilt_d,obs_watertable_d,len3,len4,Simu_dailyice, &
            &   Simu_dailyCH4,obs_CH4_MEMCMC,std_CH4,do_soilt_da,do_snow_da,do_watertable_da, &
            &   do_methane_da,do_co2_da,do_soilwater_da,len5,isimu)
...
!            write (my_fmt, '(a,i0,a)') '(i12,",",i12,",",',npara,'(F15.4,","))'
            write (my_fmt, '(a,i0,a)') '(i12,",",i12,",",7(F15.4,","))'
!                                            format(I12,",",18(F15.4,","))
            ...
            if(upgraded.gt.1000 .and. k3.lt.500)then
                ...
                if(r.gt.0.95)then
                    ...
                    if (do_methane_da) then
                        ...
                        write (644,*)'day,isimu,simuCH4_d,pro_sum_d,oxi_sum_d,fdifu1_d,ebu_sum_sat_d,pla_sum_d,&
                        & CH4V1,CH4V2,CH4V3,CH4V4,CH4V5,CH4V6,CH4V7,CH4V8,CH4V9,CH4V10,ebu_sum_unsat_d'
                        do i=1,2192
                            write(644,6044)i,isimu,(Simu_dailyCH4(j,i),j=1,17)
                        enddo
...
805                 format((i7),",",7(f15.4,","))
                    ...
                    elseif (do_co2_da) then
                        ...
                        outfile=trim(outfile)
                        outfile=adjustl(outfile)
                        open(6262,file=outfile)
                        write (6262,*)'day,isimu,GPP_d,NEE_d,Reco_d,QC(1),GL_yr,QC(2),GW_yr,&
                        & QC(3),GR_yr,QC678,pheno,LAI'
                        do i=1,2192
                            write(6262,6002)i,isimu,(Simu_dailyflux(j,i),j=1,12)
!                            write (*,*) 'i',i,'Simu_dailyflux',Simu_dailyflux(2,i)
                        enddo
6002                    format((i7),",",(i7),",",11(f15.4,","),(f15.4))
                        close(6622)
...
!*******************************************
!   ***  for write out data
      logical do_co2_da,do_methane_da,do_methane_fcast,do_EBG
      integer, parameter :: miterms=29
      real output_data(miterms,ilines)
...
!            if(yr.gt.yrs_eq)then
            ...
            Simu_dailyflux(4,daily)=QC(6)!*1.5
            Simu_dailyflux(5,daily)=GL_yr!*1.5
            Simu_dailyflux(6,daily)=QC(7)!*0.48
            Simu_dailyflux(7,daily)=GW_yr!*0.48
            Simu_dailyflux(8,daily)=QC(8)
...
!            Simu_dailyflux14(13,daily)=QC(8)!*0.48
            Simu_dailyflux14(14,daily)=QC(4)
...
!        GrowthR=MIN(GrowthP*0.35,MAX(0.0,0.75/Sw*bmL-bmR))  ! *c1/(1.+c1+c2)
     ...
     if(LAI .gt. LAIMAX .and. GrowthR+GrowthS .ne. 0)then ! added by Mary Chris
...
!               S_t(i)=Q10h(i)**((Tsoil-5.)/10.)  ! Oak
                ...
                do j=1,10       ! fast,slow and passive pool use weighed soil temperature in layers according to soc distribution
                    S_t(i)=S_t(i)+frac_soc(j)*Q10h(i)**(((j+1)-10.)/10.)  ! Duke
                enddo
             endif
          enddo
      else
...
!      write (*,*) 'QC(1))',QC(1),'OutC(1)',OutC(1),'NPP_L',NPP_L
...
!      write (*,*) 'r_me',r_me,'Q10pro',Q10pro,'kCH4',kCH4,'Omax',Omax,'CH4_thre',CH4_thre,'Tveg',Tveg,'Tpro_me',Tpro_me,'Toxi',Toxi
!      write (86,286) zwt,testout(11),Rh_pools(1),Rh_pools(2),Rh_pools(3),Rh_pools(4),Rh_pools(5),  &
!          & wsc(1),wsc(2),wsc(3),wsc(4),wsc(5),wsc(6),wsc(7),wsc(8),wsc(9),wsc(10),  &
!          & testout(2),testout(3),testout(4),testout(5),testout(6),testout(7),testout(8),  &
!          & testout(9),testout(10),testout(1),dpatm,LAIMAX,LAIMIN,phi
!286   format(31(f15.9,","))
...
!     then calculate fCH4 with CH4(i) and kCH4_layers(i)
      fCH4(i)=CH4(i)/(kCH4_layers(i)+CH4(i))   !  CH4 concentration factor
          ...
          if ((depth(i)*10.0) .le. -zwt) then                !unit of Omax: gC m-2 h-1
                  OxiCH4(i)=Omax_layers(i)*fCH4(i)*fSTO(i)*fEhO!*0.1
...
!      write (*,*) 'before difu CH4(123)', CH4(1),CH4(2),CH4(3)
...
!      write (*,*) 'Fdifu(1)',Fdifu(1),'CH4_V(1)',CH4_V(1)
...
!          write (*,*) 'before EBG CH4(5)', CH4(5)
          call ebullition_EBG(CH4,simuCH4,zwt,dpatm,tsoil_layer,wsc,THKSL,depth,  &
...
subroutine ebullition_EBG(CH4,simuCH4,zwt,dpatm,tsoil_layer,wsc,THKSL,depth,  &
                & rouwater,g,Rgas,f,Nbub,bubprob,Vmaxfraction,methanebP,methaneP,  &
                & presP,pwater,Vp,bubble_methane_tot,Ebu_sum,Ebu_sum_sat,Ebu_sum_unsat,mebu_out2)
! INPUT:
...
!    real paraest(npara,40000)
!    real paraest(5,40000)
!    integer isimu_equ
    character(len=250) indexstring
...
!   *** ..int added lines for CWE integration
    subroutine GetObsData(obs_spruce,std,obs_soilwater,obs_CH4_MEMCMC,std_CH4,     &
            &   len1,len2,obsfile1,obsfile2,obs_soilt,obsfile3,obs_watertable_d,obs_soilt_d,&
            &   obsfile4,obsfile5,obsfile6,len3,len4,len5)
!   ***
    implicit none
...
!   *** ..int
    ...
    integer istat3,istat4,len2,len3,len4
...
!   *** ..int
    ...
    open(136,file=obsfile6,status='old')
!   ***
...
!	read observed data   daily
    read (136,901) commts		!format need to be understood
    m=0
    do
        read (136,*,IOSTAT=istat5)days,CH4_flux,CH4_flux_sd,c10_mn,c10_sd,c15_mn,c15_sd,c25_mn,c25_sd,  &
        & c50_mn,c50_sd,c75_mn,c75_sd,c100_mn,c100_sd,c150_mn,c150_sd,c180_mn,c180_sd,c200_mn,c200_sd		!recble variables
        if(istat5<0) exit
                m=m+1
                obs_CH4_MEMCMC(1,m)=real(days)          !QUESTION what's real() for?
                obs_CH4_MEMCMC(2,m)=CH4_flux
                obs_CH4_MEMCMC(3,m)=c10_mn
                obs_CH4_MEMCMC(4,m)=c15_mn
                obs_CH4_MEMCMC(5,m)=c25_mn
                obs_CH4_MEMCMC(6,m)=c50_mn
                obs_CH4_MEMCMC(7,m)=c75_mn
                obs_CH4_MEMCMC(8,m)=c100_mn
                obs_CH4_MEMCMC(9,m)=c150_mn
                obs_CH4_MEMCMC(10,m)=c180_mn
                obs_CH4_MEMCMC(11,m)=c200_mn
                ...
                std_CH4(8,m)=c100_sd
                std_CH4(9,m)=c150_sd
                std_CH4(10,m)=c180_sd
                std_CH4(11,m)=c200_sd

!                if(days.lt.365*4)then
                if(days.le.2192)then                   !         Why (days.lt.365*4)
                    len5=m                              !   How many days of obs data
                endif
    enddo
...
!   Observed daily soil water data
    ...
    enddo
    len3=m-1
!   Observed daily water table data
    read(135,901) commts
    m=0
    do
        m=m+1
...
! Subroutine: Read Data assimilation check box file
    ...
    implicit none
    integer,dimension(46):: DApar
    real,dimension(46):: parmin,parmax
...
!    compute J_obs
    ...
    real Simu_dailyCH4(17,4000)
    real obs_CH4_MEMCMC(11,1000),std_CH4(11,1000)
!    integer jch4,jch4c(7),J_CH4conc(7),J_CH4flux
    integer jch4,jch4c1,jch4c2,jch4c3,jch4c4,jch4c5,jch4c6,jch4c7
    real J_CH4flux,J_CH4conc1,J_CH4conc2,J_CH4conc3,J_CH4conc4,J_CH4conc5,J_CH4conc6,J_CH4conc7,acrate
    integer isimu
...
!    J_CH4conc=(/0.,0.,0.,0.,0.,0.,0./)
    ...
    jch4=0
!    jch4c=(/0,0,0,0,0,0,0/) ! 7 layers of obs conc data
    jch4c1=0
    jch4c2=0
    jch4c3=0
    jch4c4=0
...
!    stdmc=0.5
    stdmc=0.5
    do i=1,len5			!len5=m undefined here      m is the length of simu_dailyflux
        day=int(obs_CH4_MEMCMC(1,i))        !index for simudailyCH4 data  QUESTION int()  real()
        if (obs_CH4_MEMCMC(2,i) .gt. -999) then
            jch4=jch4+1
            dObsSim=Simu_dailyCH4(1,day)-obs_CH4_MEMCMC(2,i)			!flux
!            write (*,*) isimu,day,Simu_dailyCH4(1,day),obs_CH4_MEMCMC(2,i),std_CH4(2,i)    !CH4DA
!            J_CH4flux=J_CH4flux +(dObsSim*dObsSim)/(2*std_CH4(2,i)*std_CH4(2,i))
            J_CH4flux=J_CH4flux +(dObsSim*dObsSim)/(2*stdm*stdm)
!            write (*,*) '(dObsSim*dObsSim)/(2*stdm*stdm)',(dObsSim*dObsSim)/(2*stdm*stdm)
        endif
        if (obs_CH4_MEMCMC(3,i) .gt. -999) then
            jch4c1=jch4c1+1
            dObsSim=Simu_dailyCH4(7,day)-obs_CH4_MEMCMC(3,i)			!conc layer 1
!            J_CH4conc1=J_CH4conc1+(dObsSim*dObsSim)/(2*std_CH4(3,i)*std_CH4(3,i))
            J_CH4conc1=J_CH4conc1+(dObsSim*dObsSim)/(2*stdmc*stdmc)
!        write (*,*) '(dObsSim*dObsSim)/(2*std*std)',(dObsSim*dObsSim)/(2*std_CH4(3,i)*std_CH4(3,i)),&
!        & 'dObsSim',dObsSim,'std_CH4(3,i)',std_CH4(3,i),'J_CH4conc1',J_CH4conc1
        endif
        if (obs_CH4_MEMCMC(4,i) .gt. -999) then
            jch4c2=jch4c2+1
            dObsSim=(Simu_dailyCH4(7,day)*0.5+Simu_dailyCH4(8,day)*0.5) -obs_CH4_MEMCMC(4,i)		!
!            J_CH4conc2=J_CH4conc2+(dObsSim*dObsSim)/(2*std_CH4(4,i)*std_CH4(4,i))
            J_CH4conc2=J_CH4conc2+(dObsSim*dObsSim)/(2*stdmc*stdmc)
        endif
        if (obs_CH4_MEMCMC(5,i) .gt. -999) then
            jch4c3=jch4c3+1
            dObsSim=(Simu_dailyCH4(8,day)*0.5+Simu_dailyCH4(9,day)*0.5) -obs_CH4_MEMCMC(5,i)		!
!            J_CH4conc3=J_CH4conc3+(dObsSim*dObsSim)/(2*std_CH4(5,i)*std_CH4(5,i))
            J_CH4conc3=J_CH4conc3+(dObsSim*dObsSim)/(2*stdmc*stdmc)
        endif
        if (obs_CH4_MEMCMC(6,i) .gt. -999) then
            jch4c4=jch4c4+1
            dObsSim=Simu_dailyCH4(11,day)-obs_CH4_MEMCMC(6,i)		!
!            J_CH4conc4=J_CH4conc4+(dObsSim*dObsSim)/(2*std_CH4(6,i)*std_CH4(6,i))
            J_CH4conc4=J_CH4conc4+(dObsSim*dObsSim)/(2*stdmc*stdmc)
        endif
        if (obs_CH4_MEMCMC(7,i) .gt. -999) then
            jch4c5=jch4c5+1
            dObsSim=(Simu_dailyCH4(12,day)*0.75+Simu_dailyCH4(13,day)*0.25) -obs_CH4_MEMCMC(7,i)		!
!            J_CH4conc5=J_CH4conc5+(dObsSim*dObsSim)/(2*std_CH4(7,i)*std_CH4(7,i))
            J_CH4conc5=J_CH4conc5+(dObsSim*dObsSim)/(2*stdmc*stdmc)
        endif
        if (obs_CH4_MEMCMC(8,i) .gt. -999) then
            jch4c6=jch4c6+1
            dObsSim=(Simu_dailyCH4(13,day)*0.5+Simu_dailyCH4(14,day)*0.5) -obs_CH4_MEMCMC(8,i)		!
!            J_CH4conc6=J_CH4conc6+(dObsSim*dObsSim)/(2*std_CH4(8,i)*std_CH4(8,i))
            J_CH4conc6=J_CH4conc6+(dObsSim*dObsSim)/(2*stdmc*stdmc)
        endif
        if (obs_CH4_MEMCMC(9,i) .gt. -999) then
            jch4c7=jch4c7+1
            dObsSim= Simu_dailyCH4(16,day)-obs_CH4_MEMCMC(9,i)		!
!            J_CH4conc7=J_CH4conc7+(dObsSim*dObsSim)/(2*std_CH4(9,i)*std_CH4(9,i))
...
!    J_new=J_CH4flux+J_CH4conc1+J_CH4conc2+J_CH4conc3+J_CH4conc4+J_CH4conc5+J_CH4conc6+J_CH4conc7
    J_new=J_CH4flux!+J_CH4conc1
!    J_new=J_CH4conc1+J_CH4conc2+J_CH4conc3+J_CH4conc4+J_CH4conc5+J_CH4conc6+J_CH4conc7
!    J_new=J_CH4conc1
!    J_new=J_CH4conc1+J_CH4conc4+J_CH4conc5+J_CH4conc6+J_CH4conc7
!        J_new=J_CH4flux+J_CH4conc1+J_CH4conc4+J_CH4conc5+J_CH4conc6+J_CH4conc7
   endif
...
!            &'exp(-delta_J)',exp(-delta_J),'r_num',r_num,'J_CH4conc1',J_CH4conc1
       write(*,*) 'isimu',isimu,'upgraded',upgraded,'delta_J',delta_J,'J_new',J_new,'r_num',r_num,'acrate',acrate
    return
    end
